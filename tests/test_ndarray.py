"""
Tests for isla.ndarray class - internal methods, properties, and class behavior.
"""
...
def test_construction_with_copy():
    ...
    A = isla.ndarray(original, copy=True)
    A.data[0, 0] = 999
    assert original[0, 0] == 1  # Original unchanged

    B = isla.ndarray(original, copy=False)
    B.data[0, 0] = 999
...
def test_invalid_shape():
    with pytest.raises(ValueError):
        isla.ndarray([1, 2, 3])  # Not (..., 2) shape

    with pytest.raises(ValueError):
        isla.ndarray([[1, 2, 3], [4, 5, 6]])  # Last dim not 2


def test_invalid_bounds():
    with pytest.raises(ValueError):
        isla.ndarray([[2, 1]])  # Lower > upper

    with pytest.raises(ValueError):
        isla.ndarray([[1, 2], [4, 3]])  # Second interval invalid
...
def test_shape_property():
    ...
    B = isla.ndarray([[[1, 2], [3, 4]], [[5, 6], [7, 8]]])
    assert B.shape == (2, 2)


def test_ndim_property():
    A = isla.ndarray([[1, 2]])  # 1D intervals
    assert A.ndim == 1

    B = isla.ndarray([[[1, 2], [3, 4]]])  # 2D intervals
    assert B.ndim == 2
...
def test_transpose_property():
    A = isla.ndarray([[[1, 2], [3, 4]], [[5, 6], [7, 8]]])  # 2x2
    At = A.T
...
def test_assignment():
    A = isla.ndarray([[1, 2], [3, 4]])
    new_interval = isla.ndarray([10,20])
...
def test_addition_operator():
    A = isla.ndarray([[1, 2], [3, 4]])
    B = isla.ndarray([[0.5, 1.5], [1, 2]])
    C = A + B

    assert isinstance(C, isla.ndarray)
    expected = [[1.5, 3.5], [4, 6]]
    np.testing.assert_array_equal(C.data, expected)


def test_subtraction_operator():
    A = isla.ndarray([[3, 5], [2, 4]])
    B = isla.ndarray([[1, 2], [1, 1]])
    C = A - B
...
def test_multiplication_operator():
    A = isla.ndarray([[1, 2], [2, 3]])
    B = isla.ndarray([[2, 3], [1, 2]])
    C = A * B
...
def test_division_operator():
    A = isla.ndarray([[4, 8], [6, 9]])
    B = isla.ndarray([[2, 4], [3, 3]])
    C = A / B
...
def test_matmul_operator():
    # Vector dot product
    u = isla.ndarray([[1, 2], [2, 3]])
    v = isla.ndarray([[2, 3], [1, 2]])
    result = u @ v

    assert isinstance(result, isla.ndarray)
...
def test_is_empty_method():
    ...
    # Empty intervals (NaN)
    B = isla.ndarray([[np.nan, np.nan], [1, 2]])
    result = B.is_empty()
    np.testing.assert_array_equal(result, [True, False])


def test_intersect_method():
    A = isla.ndarray([[1, 3], [5, 7]])
    B = isla.ndarray([[2, 4], [8, 9]])
    C = A.intersect(B)
...
def test_str_1d():
    A = isla.ndarray([[1, 2], [3, 4]])
    str_repr = str(A)
    assert "array([" in str_repr
    assert "[1.000, 2.000]" in str_repr
    assert "[3.000, 4.000]" in str_repr


def test_str_higher_dim():
    A = isla.ndarray([[[1, 2], [3, 4]], [[5, 6], [7, 8]]])
    str_repr = str(A)
    # Should fall back to repr for higher dimensions
    assert "array(" in str_repr
